...
def parse_lines(stream):
	...
	for line in allLines:
		...
		if(len(lineElements) == 4):
			lineElements = [lineElements[0] + " " + lineElements[1], lineElements[2], lineElements[3]]
...
def formatLines(lines):
	validLines = []
	for line in lines:
		line = [line[0], transform_index(int(line[1])), transform_index(int(line[2]))]
...
def is_linked(first, second):
	return run_bfs(first, second)

def run_bfs(first, second):
	visited = set()
	queue = [first]
	while queue:
		node = queue.pop()
